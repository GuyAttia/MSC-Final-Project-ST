...
def main(min_counts, min_cells, apply_log, batch_size, device):
    ...
    if path.isfile(path.join('/', 'data', 'NMF', 'dl_train.pth')):
        ...
        expressions, _, oe_spots, oe_genes = get_expressions(min_counts, min_cells, apply_log)
        with open(path.join('/', 'data', 'NMF', 'spots_encoder.pkl'), 'wb') as f:
            pickle.dump(oe_spots, f)
        with open(path.join('/', 'data', 'NMF', 'genes_encoder.pkl'), 'wb') as f:
